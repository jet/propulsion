module Propulsion.SqlStreamStore.Reader
...
type SliceStatsBuffer(?interval) =
    ...
    member __.Ingest(slice : ReadAllPage) =
        lock recentCats <| fun () ->
            let mutable batchBytes = 0
            for x in slice.Messages do
                let cat = categorizeEventStoreStreamId x.StreamId
            ...
            slice.Messages.Length, int64 batchBytes
...
type Range(start, sliceEnd : int64 option, ?max : int64) =
    member val Current = start with get, set

    member __.TryNext(pos : int64) =
        __.Current <- pos
        __.IsCompleted

    member __.IsCompleted =
        match sliceEnd with
        | Some send when __.Current >= send -> false
        | _ -> true
...
let chunk (pos : int64) = uint64 pos >>> 28
let posFromChunk (chunk : int) =
    let chunkBase = int64 chunk * 1024L * 1024L * 256L
    chunkBase
let posFromChunkAfter (pos : int64) =
    let nextChunk = 1 + int (chunk pos)
    posFromChunk nextChunk
let posFromPercentage (pct, max : int64) =
    let rawPos = float max * pct / 100. |> int64
...
let fetchMax (conn : IStreamStore) = async {
    let! lastItemBatch = conn.ReadAllBackwards(Position.End, 1) |> Async.AwaitTaskCorrect
    let max = lastItemBatch.FromPosition
    Log.Information("EventStore Tail Position: @ {pos} ({chunks} chunks, ~{gb:n1}GB)", max, chunk max, mb max/1024.)
    return max }
...
let establishMax (conn : IStreamStore) = async {
...
let pullStream (conn : IStreamStore, batchSize) (stream, pos, limit : int option) mapEvent (postBatch : string * StreamSpan<_> -> Async<unit>) =
    let rec fetchFrom pos limit = async {
        let reqLen = match limit with Some limit -> min limit batchSize | None -> batchSize
        let! currentSlice = conn.ReadStreamForwards(stream, pos, reqLen) |> Async.AwaitTaskCorrect
        let events = currentSlice.Messages |> Array.map (fun x -> mapEvent x)
        do! postBatch (stream.Value, { index = int64 currentSlice.FromStreamVersion; events = events })
        match limit with
        | None when currentSlice.IsEnd -> return ()
        | None -> return! fetchFrom currentSlice.NextStreamVersion None
        | Some limit when events.Length >= limit -> return ()
        | Some limit -> return! fetchFrom currentSlice.NextStreamVersion (Some (limit - events.Length)) }
    fetchFrom pos limit
...
let pullAll (slicesStats : SliceStatsBuffer, overallStats : OverallStats) (conn : IStreamStore, batchSize)
        (range:Range, once) (tryMapEvent : StreamMessage -> StreamEvent<_> option) (postBatch : int64 -> StreamEvent<_>[] -> Async<int * int>) =
    let sw = Stopwatch.StartNew() // we'll report the warmup/connect time on the first batch
    let streams, cats = HashSet(), HashSet()
    let rec aux () = async {
        let! currentSlice = conn.ReadAllForwards(range.Current, batchSize) |> Async.AwaitTaskCorrect
        ...
        let events = currentSlice.Messages |> Seq.choose tryMapEvent |> Array.ofSeq
        ...
        Log.Information("Read {pos,10} {pct:p1} {ft:n3}s {mb:n1}MB {count,4} {categories,4}c {streams,4}s {events,4}e Post {pt:n3}s {cur}/{max}",
            range.Current, range.PositionAsRangePercentage, (let e = sw.Elapsed in e.TotalSeconds), mb batchBytes,
            batchEvents, cats.Count, streams.Count, events.Length, (let e = postSw.Elapsed in e.TotalSeconds), cur, max)
        if not (range.TryNext currentSlice.NextPosition && not once && not currentSlice.IsEnd) then
            if currentSlice.IsEnd then return Eof
...
type Req =
    ...
    | Tail of seriesId : int * startPos : int64 * max : int64 * interval : TimeSpan * batchSize : int
...
type Res =
    ...
    | Batch of seriesId : int * pos : int64 * items : StreamEvent<byte[]> seq
            ...
            while true do
                ...
                if progressSw.ElapsedMilliseconds > progressIntervalMs then
                    Log.Information("Tailed {count} times @ {pos} (chunk {chunk})",
                        count, currentPos, chunk currentPos)
                    progressSw.Restart()
                count <- count + 1
