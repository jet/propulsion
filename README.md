...
- `Propulsion.CosmosStore` [![NuGet](https://img.shields.io/nuget/v/Propulsion.CosmosStore.svg)](https://www.nuget.org/packages/Propulsion.CosmosStore/) Provides bindings to Azure CosmosDB. [Depends](https://www.fuget.org/packages/Propulsion.CosmosStore) on `Equinox.CosmosStore` v `3.0.4`, `Microsoft.Azure.Cosmos` v `3.22.0`
  ...
  2. `CosmosStoreSink`: writing to `Equinox.CosmosStore` v `3.0.4`.
  3. `CosmosStorePruner`: pruning from `Equinox.CosmosStore` v `3.0.4`.
  4. `ReaderCheckpoint`: checkpoint storage for `Propulsion.Feed` using `Equinox.CosmosStore` v `3.0.4`.
...
- Metrics to support trustworthy alerting and detailed analysis of failing and stuck projections
