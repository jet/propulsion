...
- `Propulsion.CosmosStore` [![NuGet](https://img.shields.io/nuget/v/Propulsion.CosmosStore.svg)](https://www.nuget.org/packages/Propulsion.CosmosStore/) Provides bindings to Azure CosmosDb.
  a) writing to `Equinox.CosmosStore` :- `CosmosStoreSink`
  b) pruning from `Equinox.CosmosStore` :- `CosmosStorePruner`. [Depends](https://www.fuget.org/packages/Propulsion.CosmosStore) on `Equinox.CosmosStore`
  c) TODO `CosmosSource`: Not yet implemented as [the Azure Cosmos SDK Team have yet to re-expose CheckpointAsync and other such required APIs](https://github.com/jet/propulsion/issues/15)
