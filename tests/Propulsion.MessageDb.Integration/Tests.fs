...
let ``It processes events for a category`` () = async {
    ...
    let checkpoints = ReaderCheckpoint.Service("Host=localhost; Database=message_store; Port=5433; Username=postgres; Password=postgres", "public", $"TestGroup{consumerGroup}", TimeSpan.FromSeconds 10)
