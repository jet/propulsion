...
type Scenario(testOutput) =
    ...
    let stats = { new Propulsion.Streams.Stats<_>(log, TimeSpan.FromMinutes 1L, TimeSpan.FromMinutes 1L)
    ...
    let ``TailingFeedSource Stop / AwaitCompletion semantics`` () = task {
        ...
        let source = Propulsion.Feed.Core.TailingFeedSource(log, TimeSpan.FromMinutes 1L, SourceId.parse "sid", TimeSpan.FromMinutes 1L,
        ...
        do! src.Monitor.AwaitCompletion(propagationDelay = TimeSpan.FromSeconds 1L, awaitFullyCaughtUp = true)
    ...
    let SinglePassFeedSource withWait = async {
        let crawl _ _ _ = TaskSeq.singleton <| struct (TimeSpan.FromSeconds 0.1, ({ items = Array.empty; isTail = true; checkpoint = Unchecked.defaultof<_> }: Batch<_>))
        let source = SinglePassFeedSource(log, TimeSpan.FromMinutes 1L, SourceId.parse "sid", crawl, checkpoints, sink, string)
        ...
        if withWait then
            // Yields sink exception, if any
            do! src.Monitor.AwaitCompletion(propagationDelay = TimeSpan.FromSeconds 1L, awaitFullyCaughtUp = true) |> Async.ofTask
        // Yields source exception, if any
        do! src.Await()
        test <@ src.RanToCompletion @> }
