...
- MessageDb: Events technically have a global order but the store does not expose an `$all` stream equivalent. It does expose a way to read a given category in commit order. The Propulsion reader logic pulls categories independently, propulsion therefore provides no ordering guarantees across categories.
